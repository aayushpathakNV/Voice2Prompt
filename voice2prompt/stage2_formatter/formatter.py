...
class Formatter:
    ...
    def _load_model(self) -> None:
        ...
        if not Path(model_path).is_absolute():
            model_path = str(
                Path(__file__).parents[3] / "models" / model_path
            )

        logger.info(
            "loading_formatter_model",
    ...
    async def stream(self, transcript: str, queue: asyncio.Queue) -> None:
        try:
            ...
            await loop.run_in_executor(
                None, self._stream_sync, transcript, queue, loop
            )
        finally:
